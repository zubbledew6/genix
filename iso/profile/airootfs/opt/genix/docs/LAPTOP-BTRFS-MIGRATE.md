# Laptop: ext4 → btrfs + GRUB generations

**Goal:** Keep your existing LFS + Portage + Genix setup on the laptop, migrate root to btrfs `@`, enable boot-menu generations.

**Do NOT run `genix-install` for this** — that installs from the live USB root, not your laptop system. Use **btrfs-convert** (in-place) or rsync from a mounted ext4 snapshot.

**Device (yours):** `/dev/nvme0n1p3` = ext4 root, GRUB on `/dev/nvme0n1`.

---

## Before live USB (on laptop, running system)
...
tar -C /root -czf - genix-btrfs-migrate-backup | ssh zubb@MAIN_PC_IP 'cat > ~/genix-laptop-backup.tar.gz'
```
...
```bash
tar -C /home/zubb/Projects/arch-hyprland -cf - \
